...
parser.add_argument('--target_data_dir', type=str, default=os.path.dirname(os.path.abspath(__file__)) + '/../data_60/train_data.mat', help="Path to the target dataset")
...
parser.add_argument('--test_target_dir', type=str, default=os.path.dirname(os.path.abspath(__file__)) + '/../data_60/test_data.mat', help="Path to the test target dataset")
...
training_set_loss = list()
test_set_loss = list()

def train(model, optimizer, loss_fn, train_data, target_data, trsize, batch_size): #lets see what else
    """
    ...
    with tqdm(total=trsize) as pbar:
        for t in range(0, trsize, batch_size):
            ...
            targets=torch.Tensor(batch_size,1,60,60,60).cuda()
...
def train_epochs(model, optimizer, loss_fn, train_data, train_label, trsize, num_epochs, batch_size, model_dir,
    ...
    for epoch in range(num_epochs):
        ...
        train(model, optimizer, loss_fn, train_data, target_data, trsize, batch_size)
...
if __name__ == '__main__':
    ...
    target_data, tarsize = data_loader.load_data(args.target_data_dir, 'labels')
    logging.info("Number of training targets: {}".format(trsize))
    ...
    #Load test labels
    test_label, telsize= data_loader.load_data(args.test_target_dir, 'labels')
    ...
    train_epochs(autoencoder, optimizer, loss_fn, train_data, target_data, trsize, args.epochs, args.batch_size, args.model_dir,
